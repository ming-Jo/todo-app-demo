        ...
        <CardContent className="flex-1 overflow-y-auto">
          {todos.length === 0 ? (
            ...
            <div className="space-y-2">
              {todos.map(todo => (
                <TodoItem key={todo.id} todo={todo} viewMode="kanban" />
              ))}
            </div>
          )}
        </CardContent>
...
const ListView = ({ todos }: { todos: Todo[] }) => {
  ...
  return (
    <>
      {todos.map(todo => (
        <TodoItem key={todo.id} todo={todo} viewMode="list" />
      ))}
    </>
  )
}
...
const KanbanView = ({
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-4">
      {shouldShowTodoColumn && <KanbanColumn title="할 일" todos={todoItems} />}
      {shouldShowDoneColumn && <KanbanColumn title="완료" todos={doneItems} />}
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {viewMode === 'list' ? (
        ...
        <KanbanView todos={filteredTodos} filter={filter} />
      )}
    </div>
  )
}
