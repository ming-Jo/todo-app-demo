...
이 프로젝트는 [Supabase](https://supabase.com)를 사용하여 데이터를 저장합니다.

### Supabase 설정

1. **Supabase 프로젝트 생성**
   - [Supabase](https://supabase.com) 가입하고 새 프로젝트 생성(무료 플랜 500MB 스토리지)
...
- `.env` 파일은 `apps/server/.env`에 위치해야 합니다.
- Node.js 20.6.0+의 내장 `--env-file` 플래그를 사용하므로 `dotenv` 패키지가 필요 없습니다.
- `pnpm start` 또는 `pnpm dev` 실행 시 자동으로 `.env` 파일이 로드됩니다.
...
## Render 무료 플랜의 Sleep 모드

Render 무료 플랜은 15분간 요청이 없으면 sleep 모드로 전환되어 첫 요청이 느릴 수 있습니다.
